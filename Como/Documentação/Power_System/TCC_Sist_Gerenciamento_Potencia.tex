...
\begin{siglas}
  ...
  \item[LNLS] Laboratório Nacional de Luz Síncroton
...
\subsection[Bateria]{Bateria}
	...
	Para a definição da bateria foi analisado um comparativo (Tabela \ref{Tab_Bateria}) dos tipos mais comuns de baterias encontradas no mercado, sendo elas de: níquel cádmio (NiCd), níquel-hidreto metálico (NiMh), íon-lítio (Li-Íon), íon-lítio polímero (Li-Íon polímero) e chumbo (Pb).
...
\subsection[Componentes passivos]{Componentes passivos}
	...
	Os capacitores, devido as baixas pressões encontradas no espaço, não podem ser do modelo eletrolítico, ou seja, precisam ser ou cerâmicos ou de tântalo. Os capacitores cerâmicos geralmente são de 0,5 pF até 470 nF com tensão de isolação de 25 V ou 50 V. Para esses capacitores tomou-se o cuidado de selecionar os capacitores com o coeficiente de temperatura X7R, devido a sua grande faixa de temperatura de operação, conforme indicado na Figura \ref{Fig_Cap}.\textsuperscript{\cite{x7r}}
...
\subsection[Ensaio de termovácuo]{Ensaio de termovácuo}
	...
	Por fim, após todas as validações anteriores os subconjuntos foram enviados para Campinas no Laboratório Nacional de Luz Síncroton (LNLS) onde o Dr. Douglas Galante, os colocou na câmera de termovácuo.
	...
	Nesse ensaio é possível atingir baixas pressões (10\textsuperscript{-9} mbar) e também baixas temperaturas (X °C), também é possível visualizar no \textit{LabView} a curva da pressão, sendo fácil constatar se algum problema ocorreu caso na curva tenha alguma subida repentina de pressão.
