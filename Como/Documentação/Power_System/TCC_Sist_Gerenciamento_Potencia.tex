...
\section[CubeSat]{\textit{CubeSat}}
	...
	O \textit{CubeSat} Mauá, proposto no NSEE-IMT, é um equipamento do modelo 3U, sendo as unidades distribuídas em unidade de comunicação, unidade de controle de atitude e unidade de potência, como apresentado na Figura 2.
...
\subsection[Bateria]{Bateria}
	...
	Para a definição da bateria foi analisado um comparativo (Tabela 1) dos tipos mais comuns de baterias encontradas no mercado, sendo elas de: níquel cádmio (NiCd), níquel-hidreto metálico (NiMh), íon-lítio (Li-Íon), íon-lítio polímero (Li-Íon polímero) e chumbo (Pb).
...
\end{table}
	...
	Foram utilizadas duas baterias de duas células que possuem 7,4 V e 2000 mAh, sendo uma para o conjunto principal e a outra para o conjunto de redundância do Sistema de Gerenciamento de Energia para \textit{CubeSat}. Na Figura 4, é possível visualizar a bateria selecionada.
...
\subsubsection[Princípio de funcionamento]{Princípio de funcionamento}
	...
	Na Figura 5, pode ser visto o mecanismo de carga e descarga de uma bateria de íon-lítio.
...
\subsubsection[Princípio de funcionamento]{Princípio de funcionamento}
	...
	\begin{figure}[th]
		...
		\centering
		\includegraphics[width=0.7\linewidth]{./figs/fotocelula}
...
\subsection[Componentes passivos]{Componentes passivos}
	...
	\begin{figure}[th]
		...
		\centering
		\includegraphics[width=0.6\linewidth]{./figs/x7r}
	...
	\begin{figure}[th]
		...
		\centering
		\includegraphics[width=0.6\linewidth]{./figs/y5v}
...
\subsection[Validação dos subconjuntos]{Validação dos subconjuntos}
	...
	Um dos ensaios identificados foi o de validação dos subconjuntos do Sistema de Gerenciamento de Energia para \textit{CubeSat}. O circuito do sistema como um todo foi dividido em vários subconjuntos de funções especificas para serem validados, ou seja, foi separada a parte do carregador de bateria, do conversor 3,3 V, do conversor de 5 V, do conversor de 12 V e do conversor \textit{backup}. Na Figura 16 pode ser visualizado duas placas com os subconjuntos do sistema.
...
\subsection[Ensaio de radiação]{Ensaio de radiação}	
	...
	O TMP36, que é um sensor de temperatura, foi testado para \textit{SEL}, pela \textit{NASA} em 2002, pelos engenheiros espaciais Christian Poivey e Hak Kim, chegando a conclusão que a sensibilidade \textit{SEL} do componente não era um fator preocupante para aplicações espaciais.\footnote{No Anexo B é possível realizar a verificação do relatório de teste do TMP36.\textsuperscript{\cite{TMP36}}}
	
	O LMP2012, trata-se de um amplificador operacional, foi testado para \textit{SEE}, \textit{SEL}, \textit{SET} e \textit{Laser}, os testes foram realizados pelos engenheiros espaciais da \textit{NASA}, Michael Campola e Anthony Phan no final de 2012, concluindo que existem algumas áreas que param de funcionar após uma indução de \textit{laser} pulsado.\footnote{No Anexo C é possível realizar a verificação do relatório de teste do LMP2012.\textsuperscript{\cite{LMP2012}}}
	
	
\subsection[Ensaio térmico]{Ensaio térmico}

	O ensaio térmico foi realizado em parceria com o laboratório da USP, coordenado pelos Drs. Fabio Rodrigues e Douglas Galante. Esse ensaio consistia em uma câmara refrigerada que atingia uma temperatura próxima aos -80 ºC. 
